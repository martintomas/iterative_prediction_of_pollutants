...
get.data <- 'select value from input where sample=:aa and simulation=:bb order by id'
...
runBack <- function(simulation) {
  ...
  samples <- as.vector(dbGetPreparedQuery(con,'select distinct sample from input where simulation=:aa order by sample',data.frame(aa=simulation))$sample)
...
dbGetQuery(con, "create index isample on input(sample,simulation)") #create index
dbDisconnect(con)
if(num.missing.files>0) {
  mpi.close.Rslaves()
  mpi.quit()
