...
input.norm.back.addr <-  args[7] #'/home/trn/diploma-thesis/software/post-procesing/input-norm2-back.csv'
method <- 'neuralnet'
init.addr <- args[8] #"/home/trn/diploma-thesis/software/post-procesing/init-output.csv" #to set missing values
...
output.knowStations <- which(!is.na(output.reduced[1,])) #everything is known with kriging, but we compute check part only for known stations
output.reduced <- output.reduced[,apply(output.reduced, 2, function(z)all(is.finite(z)))] #remove NA

input.norm.back <- (data.matrix(read.csv(input.norm.back.addr)))
input.norm.back <- input.norm.back[,1:ncol(input)] #read only cells I need
...
mpi.bcast.Robj2slave(neuralnet) #on all nodes (function)
mpi.bcast.Robj2slave(compute) #on all nodes (function)
...
input.new <- function() { #compute new input on nodes
  ...
  for(z in groups) {
    find[,i] <- tryCatch({  
      data <- as.data.frame(cbind(input[inTrain,z],output.reduced[inTrain,corr.index[[1]][[z]]]))
      row <- paste("X",1:ncol(data),sep="")
      colnames(data) <- row
      rownames(data) <- NULL
      x <- paste(row[1],"~",paste(row[2:length(row)],collapse="+"),sep="")
      m <- neuralnet(formula=x,data=data,hidden = 3, threshold=0.01,linear.output = FALSE) 
      compute(m, output.reduced[,corr.index[[1]][[z]]])$net.result
...
output.new <- function(index) { #compute predicted output
  ...
  out <- tryCatch({
    data <- as.data.frame(cbind(output[inTrain,output.knowStations[index]],input[inTrain,corr.index[[2]][[index]]]))
    row <- paste("X",1:ncol(data),sep="")
    colnames(data) <- row
    rownames(data) <- NULL
    x <- paste(row[1],"~",paste(row[2:length(row)],collapse="+"),sep="")
    n <- neuralnet(formula=x,data=data,hidden = 3, threshold=0.01,linear.output = FALSE)     
    ...
    for(i in 1:len.samples) { #for all samples
      ...
      pred <- compute(n, input.new)$net.result #prediced values
