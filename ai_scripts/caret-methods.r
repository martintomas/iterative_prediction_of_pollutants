...
#stations.addr <- args[10]

if((method=='svmPoly') || (method=='gaussprPoly')) { #the ones which use kernlab !!!we have to load before loading data to clusters!!
  library(kernlab)
  mpi.bcast.cmd(library(kernlab))
}
...
mpi.bcast.Robj2slave(train) #on all nodes (function)
...
input.new <- function() { #compute new input on nodes
  ...
  for(z in groups) {
    find[,i] <- tryCatch({
      m <- train(output.reduced[inTrain,corr.index[[1]][[z]]], input[inTrain,z], method=method, tuneLength=tune) #use only ones which correlate, output (reduced) -> input  
...
output.new <- function(index) { #compute predicted output
  ...
  out <- tryCatch({
    n <- train(input[inTrain,corr.index[[2]][[index]]], output[inTrain,output.knowStations[index]], method=method, tuneLength=tune) #input -> output 
