...
#if [ -z "$method" ] ; then
#echo "Used caret method for script is missing"
#exit 1
#fi
...
mpirun -np 1 R --no-save -q < $DATADIR/2-version/$script --args $DATADIR/input$name.csv $DATADIR/output$name.csv $DATADIR/output$name-reduced-$REDUC.csv $DATADIR/2-version $name-$REDUC-$NUMBER $DATADIR/corr$name.db $DATADIR/input$name-back.csv $DATADIR/init-output.csv
